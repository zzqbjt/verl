...
class PRIMERewardModelWorker(Worker):
    ...
    def update_rm(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # Cache frozen-reference scores once; scoring after the update reuses them.
            self.rm.cache_reference_log_probs(data)
            ...
            if data.meta_info.get("prime_score_after_update", False):
                data.meta_info["micro_batch_size"] = self.config.micro_batch_size_per_gpu
                data.meta_info["max_token_len"] = self.config.forward_max_token_len_per_gpu
                data.meta_info["use_dynamic_bsz"] = self.config.use_dynamic_bsz
                rm_scores, _, score_metrics = self.rm.compute_rm_score(data=data)
                metrics.update(score_metrics)
                metrics["reward_model/dpo_acc"] = compute_dpo_accuracy(
                    rm_scores, acc, response_mask=response_mask, n_samples=data.meta_info["n"]
                ).detach().item()
                metrics["reward_model/dpo_acc_abs"] = compute_dpo_abs_accuracy(
