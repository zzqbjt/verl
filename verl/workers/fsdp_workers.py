...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        # normalize config
        if self._is_actor:
            # Recipes may add training responses (e.g. MC branches) without
            # changing the number of base generations in rollout.n.
            training_rollout_n = self.config.actor.get("rollout_n", self.config.rollout.n)
            self.config.actor.ppo_mini_batch_size *= training_rollout_n
